...
class eff_pose_net {
  ...
  draw_top(layer, pose) {
    ...
    let fw = abs(pose.rightEar.x - pose.leftEar.x) * r1;
    let w = fw / 8;
    let h = fh / 2;
  ...
  draw_torso(layer, pose) {
    ...
    // Arms
    this.draw_limb(layer, pose.rightElbow, pose.rightWrist, x1, y1, h);
    this.draw_limb(layer, pose.leftElbow, pose.leftWrist, x2, y2, h);

    // Legs
    let hh = h / 2;
    let hhy = hh;
    if (!this.hflip) hh = -hh;
    this.draw_limb(layer, pose.rightKnee, pose.rightAnkle, x4 - hh, y4 - hhy, h);
    this.draw_limb(layer, pose.leftKnee, pose.leftAnkle, x3 + hh, y3 - hhy, h);
  }
